...
projs["PPRPTSPG"] = projs.ix[:,"PPRPTS"].apply(lambda x: change_na(x))
projs["PPRPTS"] = projs.ix[:,"PPRPTS"].apply(lambda x: change_na(x,divide=1.0))
actual = actual.ix[:,["Name","G","FantPt","FantPprPtpg","FantPos"]]
...
def position_to_color(player):
    if position is "ALL":
        ...
        if player["FantPprPtpg"] > player["PPRPTSPG"]:
            return 1,"Above"
        else:
            return 2,"Below"
...
plt.title(year + " PPR Actual PPG Scored vs. ESPN Expected PPG - " + position)
plt.ylabel("Actual PPR PPG Scored")
plt.xlabel("ESPN Expected PPG PPR")
