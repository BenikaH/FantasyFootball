...
#read in and format ESPN actual data for 2015 season
raw_data <- read.csv("2015stats.csv", header=TRUE, dec = ".", sep = ",", na.strings = "--", stringsAsFactors = FALSE)
raw_data$Position <- factor(raw_data$Position)

#Create a plot with a regression (marked by player position)
title <- "Points Rank vs. Actual Points"
plot(raw_data$Rank, raw_data$PTS, col = raw_data$Position, main = title, xlab = "Draft Position", ylab="Points")
